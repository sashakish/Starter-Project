...
| Area | POC decision |
| --- | --- |
| **Agent hosting** | Deploy into a separate Lea & Braze-owned POC AWS account under its organization (preferred), or an isolated POC VPC if a separate account is unavailable. The consultant deploys reviewed infrastructure-as-code through a permission-bounded role limited to tagged Ava resources, with no authority over existing workloads, IAM, VPN, or routing. Use managed PostgreSQL, encrypted storage, Secrets Manager, budgets, and logs/alerts. Only the authenticated Teams webhook is internet-facing. |
...
The consultant owns application design, coding, deployment, testing, and monitoring. Work starts locally with synthetic data and mocked connections. Before anything enters Lea & Braze's AWS, the consultant provides an infrastructure plan for review; IT creates the isolated POC account or VPC and a permission-bounded deployment role. The consultant cannot change existing AWS workloads, organization-wide IAM, VPN, routing, or DNS. IT performs those company-controlled steps and creates the Microsoft, Monday, AD, and file permissions. Business reviewers provide examples, explain corrections, and judge quote quality.

| Weeks | Work split and proof before moving on |
| --- | --- |
| **1-2: Create the safe test environment** | **Consultant:** build with synthetic data first; document quote categories and baseline timing; submit the AWS infrastructure plan; then deploy only approved Ava resources and prepare the Graph, Teams, SMB, ArcGIS, Monday, and dashboard connections. **IT:** create the separate POC account or isolated VPC, permission-bounded deployment role, budget alerts, and approved network path; create the mailbox, Entra application, Teams pilot, read-only AD/file identity, output folder, and read-only Monday authorization. **Reviewers:** identify rules, escalation points, and historical cases. **Proof:** a test inquiry reaches Ava, she reads one approved file and Monday item, and responds in Teams with sending disabled; IT can disable or remove Ava without affecting existing AWS services. |
| **3-4: Build the quote agent** | **Consultant:** move case state from local files to PostgreSQL; connect the approved sources; make templates and development examples searchable without training the model on them; configure GBrain/retrieval, memory approval, ArcGIS research, quote-building tools, and Teams questions and revisions. **Reviewers:** explain representative quotes and validate sources, template selection, and draft structure. **Proof:** Ava replays development cases, cites the material used, answers case questions, and produces reviewable draft packages without contacting customers. |
| **5-6: Test and make it dependable** | **Consultant:** run untouched holdout cases; correct failures; add identity and permission checks, exact-version approval, duplicate and attachment protection, retries, audit logs, alerts, backup/restore, and emergency shutoff. Test unclear Teams requests and verify that proposed memories cannot activate without approval. **Reviewers:** score drafts, record corrections, and approve or reject proposed learning. **Proof:** the holdout results and safety tests meet the agreed threshold for trying real inquiries in parallel. |
| **7-8: Compare Ava with live work** | **Team:** continue the normal quote process while Ava receives copies of an initial 15-25 eligible inquiries and independently prepares drafts. **Consultant:** monitor every case, measure elapsed and hands-on time, record corrections and cost, and report results by quote family, jurisdiction, and layout. Ava does not send customer email unless Jim and IT separately approve a monitored send test. **Proof:** final scorecard, coverage gaps, risk register, operating runbook, and a go, revise, or stop recommendation. |
